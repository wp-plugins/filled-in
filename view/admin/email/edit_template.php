...
<form id="template_form_<?php echo $template->name ?>" method="post" action="<?php echo $this->url () ?>/controller/admin_ajax.php?cmd=update_template&amp;id=<?php echo $template->name ?>&amp;ajax_nonce=<?php echo wp_create_nonce ('filledin-save_template');?>">
		<h3><?php printf (__ ("Editing template '%s'", 'filled-in'), $template->name); ?></h3>
	  ...
	  <table width="100%" class="form-table">
	    <tr>
	      ...
	      <td><input class="regular-text" type="text" name="from" value="<?php echo htmlspecialchars (stripslashes ($template->from)) ?>" size="40"/> (<?php _e ('Leave blank for admin email address', 'filled-in') ?>)</td>
	    </tr>
	    <tr>
	      <th><?php _e ('Subject', 'filled-in'); ?>:</th>
	      <td><input class="regular-text" type="text" name="subject" value="<?php echo htmlspecialchars (stripslashes ($template->subject)) ?>" size="40"/></td>
	    </tr>
	    ...
	    <tr>
	      ...
	      <td>
					<textarea class="large-text" name="html" rows="10"><?php echo htmlspecialchars (stripslashes ($template->html)) ?></textarea>
				</td>
	    </tr>
	    ...
	    <tr <?php if (strlen ($template->text) == 0) echo 'style="display: none"'; ?> id="text_<?php echo $template->name ?>">
	      ...
	      <td><textarea class="large-text" name="text" rows="10"><?php echo htmlspecialchars (stripslashes ($template->text)) ?></textarea></td>
	    </tr>
